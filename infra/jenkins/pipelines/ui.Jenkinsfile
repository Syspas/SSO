pipeline {
    ...
    post {
        always {
            script {
                if (sh(script: 'ls target/surefire-reports-ui/*.xml >/dev/null 2>&1', returnStatus: true) == 0) {
                    junit testResults: 'target/surefire-reports-ui/*.xml'
                }
                if (fileExists('target/allure-results')) {
                    ...
                    sh 'jar --create --no-manifest --file allure-report-ui.zip -C target/allure-report .'
                    archiveArtifacts artifacts: 'allure-report-ui.zip', allowEmptyArchive: false
                }
            }
        }
    }
